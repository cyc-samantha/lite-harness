...
7. [KEPT] **EVERY PIPELINE PRODUCES AN OBSERVATION.** No exceptions — successes and failures both. Lite status: kept, minimal — the orchestrator appends one JSONL line (`{ts, idea, phases, loops, outcome}`) to `${CLAUDE_PLUGIN_DATA:-${CLAUDE_CONFIG_DIR:-$HOME/.claude}}/lite/observations.jsonl`. No processing loop consumes it in lite — capture only.
