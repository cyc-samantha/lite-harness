...
- **A slug is given**: report on `${CLAUDE_PLUGIN_DATA:-${CLAUDE_CONFIG_DIR:-$HOME/.claude}}/lite/runs/<slug>/STATE.md` only. If it does not exist, say so and stop.
- **`all` or no argument**: report on every `${CLAUDE_PLUGIN_DATA:-${CLAUDE_CONFIG_DIR:-$HOME/.claude}}/lite/runs/*/STATE.md` that exists, most-recently-active first (sort by `last_seen` if present, else `created`).
- If `${CLAUDE_PLUGIN_DATA:-${CLAUDE_CONFIG_DIR:-$HOME/.claude}}/lite/runs/` has no runs at all, print "No lite runs found." and stop.
